...
func TestIndexFromTail(t *testing.T) {
	...
	Dll1 := buildList("a1", "a2", "a3", "a4", "a5")

	rv, err := Dll1.IndexFromTail(0)
	if err != nil || rv.Data.S != "a5" {
		t.Errorf("IndexFromTail(0): expected a5, got %v, %v", rv, err)
	}
	rv, err = Dll1.IndexFromTail(2)
	if err != nil || rv.Data.S != "a3" {
		t.Errorf("IndexFromTail(2): expected a3, got %v, %v", rv, err)
	}
	rv, err = Dll1.IndexFromTail(4)
	if err != nil || rv.Data.S != "a1" {
		t.Errorf("IndexFromTail(4): expected a1, got %v, %v", rv, err)
	}
	if _, err = Dll1.IndexFromTail(5); err == nil {
		t.Errorf("Expected error from IndexFromTail(5)")
	}
	if _, err = Dll1.IndexFromTail(-1); err == nil {
		t.Errorf("Expected error from IndexFromTail(-1)")
	}

	var Dll2 Dll[TestDemo]
	if _, err = Dll2.IndexFromTail(0); err == nil {
		t.Errorf("Expected error from IndexFromTail on empty list")
	}
}
