...
func (ns *Dll[T]) UnmarshalJSON(data []byte) error {
	...
	if len(items) > 0 {
		if ns == nil {
			panic("dll: UnmarshalJSON called on a nil list")
		}
		if ns.eq == nil {
			panic("dll: UnmarshalJSON called on a list with no equality function (create the list with NewDll or NewDllFunc)")
		}
	}
	if ns == nil {
