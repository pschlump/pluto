...
// the digraph are not reflected, and a DirectedCycle is safe for
// concurrent reads.
...
// NewDirectedCycle snapshots the digraph's adjacency lists under the read
// lock, then searches the snapshot lock-free for a directed cycle.  A nil
// or empty digraph has no cycle (HasCycle is false and Cycle is nil) —
// there is a sane answer, so nothing panics.
