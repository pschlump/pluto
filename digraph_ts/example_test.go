...
func ExampleNewDirectedDFS() {
	g := digraph_ts.NewDigraph(6)
	for _, e := range [][2]int{{0, 1}, {1, 2}, {2, 0}, {2, 3}, {4, 5}} {
		g.AddEdge(e[0], e[1])
	}

	d := digraph_ts.NewDirectedDFS(g, 0, 4)
	fmt.Print("reachable:")
	for v := 0; v < g.V(); v++ {
		if d.Marked(v) {
			fmt.Printf(" %d", v)
...
func ExampleNewBFSDirectedPaths() {
	...
	p := digraph_ts.NewBFSDirectedPaths(g, 0)
	for v := 0; v < g.V(); v++ {
		d, _ := p.DistTo(v)
		path, _ := p.PathTo(v)
...
func ExampleNewDirectedCycle() {
	...
	c := digraph_ts.NewDirectedCycle(g)
...
func ExampleNewTopological() {
	dag := digraph_ts.NewDigraph(5)
	for _, e := range [][2]int{{0, 1}, {0, 2}, {1, 3}, {2, 3}, {3, 4}} {
		dag.AddEdge(e[0], e[1])
	}
	top := digraph_ts.NewTopological(dag)
	fmt.Println("DAG order:", top.Order())

	cyclic := digraph_ts.NewDigraph(3)
	for _, e := range [][2]int{{0, 1}, {1, 2}, {2, 0}} {
		cyclic.AddEdge(e[0], e[1])
	}
	fmt.Println("cyclic has order:", digraph_ts.NewTopological(cyclic).HasOrder())
	// Output:
	// DAG order: [0 2 1 3 4]
