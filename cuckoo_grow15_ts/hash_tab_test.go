...
// a known base hash each occupied slot must be one of the four
// shifted/reduced candidates.  The internals are inspected under Lock; no
// background resizer is active (the thresholds are never crossed).
func TestPositionsDerivedFromBaseHash(t *testing.T) {
	...
	for i := range ht.slots {
		...
		ok := false
		for j := 0; j < 4; j++ {
			if i == posOf(ht.slots[i].hash, j, size) {
				ok = true
				break
			}
		}
	...
	ht.Unlock()
	if bad >= 0 {
		t.Fatalf("slot %d holds h=%d which is not one of its own four candidates for size %d", bad, badHash, size)
	}
}

// TestCapacityIsPowerOfTwo verifies the constructor honors the requested
// size and the minimum size (sizes need no longer be powers of two — the
// multiply-shift addressing works for any size).
func TestCapacityIsPowerOfTwo(t *testing.T) {
	for _, n := range []int{5, 8, 9, 13, 16, 17, 100, 129, 1000} {
		ht := newTestTab(n, 0, 0)
		c := ht.Capacity()
		if c < minTableSize {
			t.Errorf("n = %d: capacity %d is below the minimum %d", n, c, minTableSize)
		}
		if c < n {
