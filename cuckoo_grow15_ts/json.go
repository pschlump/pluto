...
func (tt *HashTab[T]) UnmarshalJSON(data []byte) error {
	...
	if len(items) > 0 {
		if tt == nil {
			panic("cuckoo_grow15_ts: UnmarshalJSON called on a nil table")
		}
		if tt.eq == nil || tt.hash == nil {
			panic("cuckoo_grow15_ts: UnmarshalJSON called on a table with no equality/hash functions (create the table with NewHashTab or NewHashTabFunc)")
		}
	}
	if tt == nil {
