...
// methods: an atomic search-then-update sequence under the manual lock.
// The compound operation uses the Nl (no-lock) methods because the
// locked public methods must not be called while the lock is held.
func TestLockNlCompound(t *testing.T) {
	...
	pq.Lock()
	// Atomically find the priority-20 element and demote it to the front.
	if v, pos, found := pq.NlSearch(PqTest{priority: 20}); found {
		if !pq.NlUpdatePriority(pos, PqTest{value: v.value, priority: 1}) {
			t.Errorf("NlUpdatePriority(%d) reported false on valid position.", pos)
		}
	}
	pq.NlInsert(PqTest{value: "c", priority: 30})
	n := pq.NlLen()
	if !pq.NlIsEmpty() {
		if _, found := pq.NlPeek(); !found {
			t.Errorf("Expected NlPeek on non-empty queue to report true.")
		}
	}
	pq.Unlock()

	if n != 3 {
		t.Errorf("Expected NlLen 3 under the lock, got %d", n)
	}
	if v, found := pq.Peek(); !found || v.priority != 1 || v.value != "b" {
		t.Errorf("Peek after compound = (%v, %v), want b/1", v, found)
	}

	// Out-of-range Nl ops report false.
	pq.Lock()
	if _, found := pq.h.NlGetValue(-1); found {
		t.Errorf("Expected NlGetValue(-1) to report false.")
	}
	if _, found := pq.NlDelete(pq.NlLen()); found {
		t.Errorf("Expected NlDelete(len) to report false.")
	}
	if pq.NlUpdatePriority(pq.NlLen(), PqTest{priority: 9}) {
		t.Errorf("Expected NlUpdatePriority(len) to report false.")
	}
	pq.Unlock()
	if pq.Len() != 3 {
		t.Errorf("Invalid Nl ops changed length to %d", pq.Len())
	}
}
...
// TestPQIterateSnapshot verifies that the All iterator operates on a
// snapshot taken when it is called: later modifications — even truncating
// the whole queue — are not observed, and mutating from inside the loop
// is safe.
func TestPQIterateSnapshot(t *testing.T) {
	pq := newTestPQ(
		PqTest{value: "a", priority: 5},
		PqTest{value: "b", priority: 2},
		PqTest{value: "c", priority: 8},
...
// TestPQConcurrent runs concurrent inserters, poppers, readers,
// search-then-update/delete users and a Lock/Unlock batch user against
// one shared queue.  It is meant to be run with the race detector
// (`make race`); the point is to detect data races, deadlocks and
// panics, not exact interleavings.  Afterwards the queue must still
// drain in non-decreasing priority order.
func TestPQConcurrent(t *testing.T) {
	...
	for range 4 {
		wg.Go(func() {
			for range 200 {
				pq.Pop() // not-found on empty is fine
			}
		})
	}
	...
	// Search-then-update/delete users (positions may race; false returns
	// are fine).
	wg.Go(func() {
		for i := range 100 {
			probe := PqTest{priority: i % 97}
			if _, pos, found := pq.Search(probe); found {
	...
	// The queue must still drain in sorted order (single goroutine now,
	// so the order check cannot race on recordings).
	prev := -1
	for pq.Len() > 0 {
		got, found := pq.Pop()
		if !found {
			t.Fatal("Pop reported false on a non-empty queue")
		}
		if got.priority < prev {
