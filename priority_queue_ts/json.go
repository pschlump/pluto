...
// the same snapshot All builds (copied out under the heap's write lock),
// so the queue is unchanged afterwards and the encoding itself runs
// without the lock, safe to call concurrently with any queue operation.
//
...
// the queue under one hold of the write lock.  The array order does not
// matter to the queue — elements are inserted one by one and come back
// out in priority order — but the comparison function is kept, so the
// queue stays usable after unmarshaling.
//
...
// wrong element types) is returned with the queue untouched.  The
// nil/zero-value guard is also checked before the lock is acquired (h is
// set only by the constructors, so reading it unlocked is safe).
//
...
func (pq *PriorityQueue[T]) UnmarshalJSON(data []byte) error {
	...
	if len(items) > 0 {
		if pq == nil || pq.h == nil {
			panic("priority_queue_ts: UnmarshalJSON called on a nil or zero-value queue (create the queue with NewPriorityQueue or NewPriorityQueueFunc)")
		}
	}
	if pq == nil || pq.h == nil {
