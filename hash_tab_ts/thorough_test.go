...
// Thorough tests for the thread-safe chained hash table: nil and zero-value
// behavior, the panic contract with its messages, replacement semantics,
// chain construction and unlinking, the fixed bucket count, snapshot
// iterator semantics, compound Lock+Nl operations, a randomized cross-check
// against a map model, and concurrent access.  TestData and newTestTab are
...
// — in single-goroutine tests only (it reads the internals without the
// lock).
...
// middle, the head and the tail of the chain.
...
func TestUnmarshalJSON(t *testing.T) {
	...
	for _, want := range []TestData{{S: "x", N: 1}, {S: "y", N: 2}} {
		if got, found := ht.Search(TestData{S: want.S}); !found || got != want {
			t.Errorf("Expected to find %v, got (%v, %v)", want, got, found)
		}
	}
	checkInvariants(t, ht)
	...
	if v, found := again.Search(TestData{S: "c"}); !found || v.N != 2 {
		t.Errorf("Expected Search to work after unmarshal, got (%v, %v)", v, found)
	}
	...
	if v, found := dup.Search(TestData{S: "k"}); !found || v.N != 2 {
		t.Errorf("Expected the last duplicate to win, got (%v, %v)", v, found)
	}
	...
	for _, badData := range []string{"[1,", `["x"]`, `{"S":"a"}`, "7", `[{"S":"a"},3]`} {
		...
		if v, found := keep.Search(TestData{S: "keep"}); !found || v.N != 1 {
...
func TestJSONRandomizedModel(t *testing.T) {
	...
	for step := range ops {
		...
		for mk, mv := range model {
			if v, found := fresh.Search(TestData{S: mk}); !found || v.N != mv {
				t.Fatalf("step %d: round trip Search(%q) = (%v, %v), model %d", step, mk, v, found, mv)
			}
		}
	}
	checkInvariants(t, ht)
