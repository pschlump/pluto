...
// call other table methods from the loop body.  Bucket order depends on the
// per-table hash seed, so it varies from process to process — never assert
// a fixed order.
// Complexity is O(n).
func (tt *HashTab[T]) All() iter.Seq2[int, T] {
	...
	for i := range tt.buckets {
		for node := tt.buckets[i]; node != nil; node = node.next {
			snap = append(snap, pair{pos: i, item: node.data})
		}
	}
	tt.lock.RUnlock()
