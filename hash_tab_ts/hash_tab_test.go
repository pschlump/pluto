...
// Thread-safe fixed-size chained hash table.
...
// and one line per stored element (a chain prints one line per element).
func TestDumpOutput(t *testing.T) {
	...
	// Element lines carry the hash, empty buckets are marked empty.
	occupied, empty := 0, 0
	for _, line := range strings.Split(out, "\n")[1:] {
		if strings.Contains(line, "empty") {
			empty++
		} else if strings.Contains(line, "h=") {
			occupied++
		}
	}
...
// and that Truncate clears all internal state (no stale chains).
func TestTruncateReuse(t *testing.T) {
	...
	for i := range ht.buckets {
		if ht.buckets[i] != nil {
			t.Fatalf("Expected nil chain after Truncate, bucket %d is not", i)
		}
	}
...
// the chain and unlink paths under heavy collision (size 7, keys 0..399).
