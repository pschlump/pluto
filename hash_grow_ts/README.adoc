...
It provides an API identical to link:../hash_grow[`hash_grow`]; see that package
...
. the `Hashable` interface (`HashKey(x any) int`), if the element implements it,
. the element itself if it is a `string`,
. `fmt.Stringer`, hashed with FNV-1a.
