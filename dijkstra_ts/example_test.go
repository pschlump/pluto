...
func Example() {
	...
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 1, To: 3, Weight: 6})
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 2, To: 3, Weight: 3})
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 3, To: 4, Weight: 1})

	sp := dijkstra_ts.NewDijkstraSP(g, 0)
	for v := 0; v < g.V(); v++ {
		fmt.Printf("0 -> %d: %g\n", v, sp.DistTo(v))
	}
...
func ExampleDijkstraSP_DistTo() {
	...
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 0, To: 1, Weight: 4})
...
func ExampleNewDijkstraUndirectedSP() {
	...
	g.AddEdge(dijkstra_ts.Edge{V: 0, W: 1, Weight: 7})
	g.AddEdge(dijkstra_ts.Edge{V: 1, W: 2, Weight: 2})
	g.AddEdge(dijkstra_ts.Edge{V: 2, W: 3, Weight: 2})
	g.AddEdge(dijkstra_ts.Edge{V: 0, W: 3, Weight: 20})

	sp := dijkstra_ts.NewDijkstraUndirectedSP(g, 3)
	fmt.Printf("3 -> 0: %g\n", sp.DistTo(0))
...
func ExampleNewDijkstraAllPairsSP() {
	...
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 1, To: 2, Weight: 2})
	g.AddEdge(dijkstra_ts.DirectedEdge{From: 0, To: 2, Weight: 10})

	ap := dijkstra_ts.NewDijkstraAllPairsSP(g)
	for s := 0; s < g.V(); s++ {
		for t := 0; t < g.V(); t++ {
			if t > 0 {
				fmt.Print("  ")
