...
list link:../sll_ts[`sll_ts`].  Elements are stored and returned by pointer.
...
the link:../comparable[`comparable.Equality`] interface (a requirement
...
import (
	...
	stack "github.com/pschlump/pluto/stack_sll_ts" // the package is named "stack"
)
