...
// JSON object mapping each key to its value; the json package emits
// object members in sorted key order, which is the trie's natural
// iteration order (All).
//
...
// Complexity is O(n) plus the cost of encoding the values.
func (t *PatriciaTrie[T]) MarshalJSON() ([]byte, error) {
	if t == nil {
		return []byte("{}"), nil
	}
...
// contents of the trie.  The trie needs no constructor-set functions,
// so it stays usable after unmarshaling no matter how it was created.
//
...
// values.
func (t *PatriciaTrie[T]) UnmarshalJSON(data []byte) error {
	var m map[string]T
	...
	if len(m) > 0 && t == nil {
		panic("patricia_trie: UnmarshalJSON called on a nil PatriciaTrie")
	}
	if t == nil {
