...
func ExampleCompare() {
	fmt.Println(avl_tree.Compare("apple", "banana"))
	fmt.Println(avl_tree.Compare(3.5, 1.5))
	fmt.Println(avl_tree.Compare(7, 7))
	// Output:
	// -1
	// 1
	// 0
...
func ExampleAvlTree_Truncate() {
	tree := avl_tree.NewAvlTreeFunc(func(a, b int) int {
		return a - b
	})
	tree.Insert(1)
	tree.Insert(2)
...
// works: it adopts the first operand's comparison function.
func ExampleAvlTree_Union() {
	a := avl_tree.NewAvlTree[int]()
	for _, v := range []int{1, 2, 3} {
		a.Insert(v)
	}
	b := avl_tree.NewAvlTree[int]()
	for _, v := range []int{2, 3, 4} {
		b.Insert(v)
	}

	var dst avl_tree.AvlTree[int] // zero value: adopts a's comparison function
	dst.Union(a, b)
