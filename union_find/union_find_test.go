...
// TestUnionByRankShape verifies that merging singleton pairs and then
// the pairs keeps the forest shallow (rank growth is logarithmic): with
// 8 elements fully merged, every element is at most 3 links from the
// root.
func TestUnionByRankShape(t *testing.T) {
	const n = 8
	uf := NewUnionFind(n)
	uf.Union(0, 1)
	uf.Union(2, 3)
	...
	uf.Union(0, 4)
	if uf.Count() != 1 {
		t.Fatalf("Expected Count()=1, got %d", uf.Count())
	}
	root, _ := uf.Find(0)
	for i := range n {
		depth := 0
		for p := i; uf.parent[p] != p; p = uf.parent[p] {
			depth++
		}
		if depth > 3 {
			t.Errorf("Element %d is %d links from the root; union-by-rank should keep it ≤ 3", i, depth)
		}
		if r, _ := uf.Find(i); r != root {
			t.Errorf("Expected Find(%d)=%d, got %d", i, root, r)
		}
	}
}
