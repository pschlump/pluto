...
// on the forward pointers (see skip_list.go).  Both operations are
// O(log₂ n + m) — one descent to the edge of the span, then one pass over
// the m removed elements.
...
// merely loses the k skipped nodes.  The caller is responsible for the run
// being non-empty.
