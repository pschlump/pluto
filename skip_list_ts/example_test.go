...
// Range iterates a snapshot of the elements x with lo <= x <= hi in
// ascending order, yielding each element's global rank alongside its value;
// it is safe to mutate the list while the loop runs.
