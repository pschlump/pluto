...
func mruOrder[K comparable, V any](c *lru_ts.Lru[K, V]) []K {
	keys := []K{}
	for k := range c.All() {
		keys = append(keys, k)
	}
...
// least recently used entry is evicted first.  (Port of the plain
// package's TestGetPutEvict — behavioral identity.)
func TestGetPutEvict(t *testing.T) {
	c := lru_ts.NewLru[string, int](3)
	c.Put("a", 1)
	c.Put("b", 2)
	c.Put("c", 3)
...
func TestPutUpdateMarksMRU(t *testing.T) {
	...
	if _, ok := c.Get("b"); ok {
		t.Error("Get(b) after eviction found b")
	}
}

// Peek does not reorder; Get does — the Get-pinned entry survives the
// eviction that the Peek-ed entry falls to.  This is also the
// write-lock-vs-read-lock split in action: Peek is a true read.
...
func TestVetoSkipsProtected(t *testing.T) {
	...
	c := lru_ts.NewLruFunc(2, func(k string, _ int) bool { return !protected[k] })
	c.Put("a", 1)
	c.Put("b", 2)
...
func TestAllVetoedSoftCap(t *testing.T) {
	vetoAll := true
	c := lru_ts.NewLruFunc(2, func(_ string, _ int) bool { return !vetoAll })
	c.Put("a", 1)
	c.Put("b", 2)
...
func TestNilAndZeroValueTolerated(t *testing.T) {
	...
	for _, c := range []*lru_ts.Lru[string, int]{nilCache, &zeroCache} {
...
// The full panic contract, in the twin's own voice (lru_ts-prefixed):
// bad capacity at construction, Put on a nil cache, Put on a
// zero-value cache.
func TestPanics(t *testing.T) {
	expectPanicMsg(t, "NewLru(0)", func() { lru_ts.NewLru[int, int](0) }, "lru_ts:", "capacity")
	expectPanicMsg(t, "NewLruFunc(-1)", func() { lru_ts.NewLruFunc[int, int](-1, nil) }, "lru_ts:", "capacity")

	var nilCache *lru_ts.Lru[string, int]
	expectPanicMsg(t, "Put on a nil cache", func() { nilCache.Put("k", 1) }, "lru_ts:", "Put", "NewLru")

	var zeroCache lru_ts.Lru[string, int]
	expectPanicMsg(t, "Put on a zero-value cache", func() { zeroCache.Put("k", 1) }, "lru_ts:", "Put", "NewLru")
}
