...
func TestDefaultConstructor(t *testing.T) {
	...
	a := NewHashTab[int](11, 0)
	b := NewHashTab[int](11, 0)
	for i := range 50 {
		a.Insert(i * 7)
		b.Insert(i * 7)
	}
...
func TestDumpOutput(t *testing.T) {
	...
	if n := strings.Count(out, "\n"); n != 1+ht.size { // header + one line per bucket (the table grew past 7 buckets)
		t.Errorf("Expected %d lines from Dump, got %d", 1+ht.size, n)
	}
	for i := range 20 {
	...
	// Occupied lines carry the hash, empty lines are marked empty.
	occupied, empty := 0, 0
	...
	if occupied != 20 || empty != ht.size-20 {
		t.Errorf("Expected 20 occupied and %d empty bucket lines, got %d/%d", ht.size-20, occupied, empty)
	}
}
