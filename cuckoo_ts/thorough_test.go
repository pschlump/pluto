...
// background grow and shrink resizes exactly.
func TestInfoCounters(t *testing.T) {
	...
	// Growth is asynchronous in this package: inserts keep running against
	// the saturated table until the background rebuild lands, so a fast
	// insert burst can legitimately hit the kick limit first.  Forced is
	// therefore not required to be 0 — only bounded by Grows.
	...
	// Shrink: delete down to a single element; the background resizer
	// halves the table back to the minimum size.
	for i := 0; i < 999; i++ {
		ht.Delete(i)
	}
	waitQuiescent(t, ht)
...
// TestInfoForcedCountsCollisionLoops checks that the synchronous resizes a
// pathological hash forces inside Insert are counted as Forced (and as
// Grows) — one per escalation, maxResizeAttempts of them before the panic.
