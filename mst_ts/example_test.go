...
import (
	...
	"github.com/pschlump/pluto/mst_ts"
)

// byEdge orders edges by weight, then by endpoints, so example output
// is deterministic.
func byEdge(a, b dijkstra_ts.Edge) int {
...
func ExampleNewLazyPrimMST() {
	g := dijkstra_ts.NewEdgeWeightedGraph(5)
	g.AddEdge(dijkstra_ts.Edge{V: 0, W: 1, Weight: 4})
	g.AddEdge(dijkstra_ts.Edge{V: 1, W: 2, Weight: 5})
	g.AddEdge(dijkstra_ts.Edge{V: 3, W: 4, Weight: 6})

	mst := mst_ts.NewLazyPrimMST(g) // components {0,1,2} and {3,4}
	edges := mst.Edges()
	slices.SortFunc(edges, byEdge)
	for _, e := range edges {
		fmt.Printf("%d--%d (%g)\n", e.V, e.W, e.Weight)
