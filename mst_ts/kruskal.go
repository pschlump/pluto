...
// NewKruskalMST snapshots the graph's adjacency lists (each under the
// graph's read lock), then computes a minimum spanning tree lock-free
// on the snapshot with Kruskal's algorithm: sort every edge by weight
// and greedily add each edge that does not close a cycle, tracked with
// a union-find (pluto/union_find).
//
...
// space.
func NewKruskalMST(g *dijkstra_ts.EdgeWeightedGraph) *KruskalMST {
	...
	mst := &KruskalMST{}
	uf := union_find.NewUnionFind(len(adj))
	for _, e := range all {
...
// Complexity is O(Len).
func (m *KruskalMST) Edges() []dijkstra_ts.Edge {
	if m == nil {
		return nil
	}
