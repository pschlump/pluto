...
// Non-locking version of cuckoo table.
...
func TestPositionsDerivedFromBaseHash(t *testing.T) {
	...
	for i := range ht.slots {
		...
		if !ok {
			t.Fatalf("slot %d holds h=%d which derives candidates %d,%d,%d,%d for mask %d",
				i, ht.slots[i].hash,
				posOf(ht.slots[i].hash, 0, mask), posOf(ht.slots[i].hash, 1, mask),
				posOf(ht.slots[i].hash, 2, mask), posOf(ht.slots[i].hash, 3, mask), mask)
		}
	}
}
...
func TestCapacityIsPowerOfTwo(t *testing.T) {
	for _, n := range []int{5, 8, 9, 13, 16, 17, 100, 129, 255, 256, 257, 1000} {
		ht := newTestTab(n, 0, 0)
		c := ht.Capacity()
	...
	if c := newTestTab(200, 0, 0).Capacity(); c != minTableSize {
		t.Errorf("a request below the minimum rounds up to %d, want %d", c, minTableSize)
	}
}

// TestDefaultConstructor exercises NewHashTab (random maphash seed) for
// membership only — placement is nondeterministic.
func TestDefaultConstructor(t *testing.T) {
	...
	if ht.Saturation() > 0.85 {
		t.Errorf("saturation %.4f above the 0.85 grow threshold after inserts", ht.Saturation())
	}
}

// TestHashOfExactlyZero needs no remapping: the occupied flag marks a slot,
// so a base hash of exactly 0 is stored and compared as-is.  Note that with
// an all-zero hash all four candidates are slot 0, so only one such element
// can ever place — the many-elements case is TestPathologicalSameHash.
func TestHashOfExactlyZero(t *testing.T) {
	...
	ht.Insert(7)
	if ht.slots[0].hash != 0 || !ht.slots[0].used {
		t.Fatalf("the zero hash should be stored as-is in slot 0, got used=%v", ht.slots[0].used)
	}
...
func TestDumpOutput(t *testing.T) {
	...
	if !strings.Contains(out, "{a") {
		t.Errorf("Dump should show the stored elements, got:\n%s", out)
	}
}
...
func TestTruncateReuse(t *testing.T) {
	...
	for i := range ht.slots {
		if ht.slots[i].used {
			t.Fatalf("slot %d still used after Truncate", i)
		}
	}
