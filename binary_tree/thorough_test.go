...
func TestMarshalJSON(t *testing.T) {
	// Exact array output in in-order (ascending) order, regardless of
	// insertion order.
	...
	custom := NewBinaryTree[upperString]()
	custom.Insert("x")
	custom.Insert("y")
...
func TestUnmarshalJSON(t *testing.T) {
	// Decoded elements are inserted in array order; the tree iterates in
	...
	if min, found := tree.FindMin(); !found || min != 1 {
		t.Errorf("Expected FindMin 1, got (%v, %v)", min, found)
	}
	...
	// Duplicates in the array collapse, as with Insert.
	dup := NewBinaryTree[int]()
	if err := json.Unmarshal([]byte("[2,1,2,1]"), dup); err != nil {
		t.Fatalf("json.Unmarshal: %v", err)
	}
	if got, want := dup.Len(), 2; got != want {
		t.Errorf("Expected duplicates to collapse to length %d, got %d", want, got)
	}
...
// TestJSONStructField marshals and unmarshals a BinaryTree nested in a
// struct through the encoding/json package.  The tree must be created
// with NewBinaryTree/NewBinaryTreeFunc before unmarshaling: for a nil
...
func TestJSONRandomizedModel(t *testing.T) {
	...
	sortedModel := func() []int {
		keys := []int{} // non-nil, so an emptied model marshals as [] like the tree
		for k := range model {
			keys = append(keys, k)
		}
		sort.Ints(keys)
	...
	for step := range ops {
		k := rng.Intn(100)
		switch rng.Intn(5) {
		case 0, 1, 2: // Insert (duplicates replace in both tree and model)
			tree.Insert(k)
			model[k] = true
		case 3: // Delete
			tree.Delete(k)
			delete(model, k)
		case 4: // DeleteAtHead
			if keys := sortedModel(); len(keys) > 0 {
				tree.DeleteAtHead()
				delete(model, keys[0])
			}
		}
		...
		// Marshal must equal the sorted distinct keys marshaled as a slice.
		...
		if string(got) != string(wantJSON) {
			t.Fatalf("step %d: marshaled %s, model %s", step, got, wantJSON)
		}
		...
		if fmt.Sprint(vals) != fmt.Sprint(want) {
			t.Fatalf("step %d: round trip got %v, model %v", step, vals, want)
		}
	}
}
