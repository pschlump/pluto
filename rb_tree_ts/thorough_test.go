...
// TestTreeIterateSnapshot verifies that the All/Backward iterators operate
// on a snapshot taken when they are called: later modifications — even
// truncating the whole tree — are not observed, and mutating the tree from
// inside the loop is safe.
func TestTreeIterateSnapshot(t *testing.T) {
	tree := newTestTree()
	for _, s := range []string{"05", "02", "09", "00", "03"} {
		tree.Insert(TestRbTreeNode{S: s})
	}
...
// ends up empty, ordered and consistent.
func TestTreeConcurrent(t *testing.T) {
	tree := NewRbTreeFunc(cmpTestRbTreeNode)
	...
	for w := range workers {
		...
		go func(w int) {
			...
			for i := range perWorker {
				k := TestRbTreeNode{S: fmt.Sprintf("%02d-%04d", w, i)}
				if !tree.Insert(k) {
...
func TestUnmarshalJSON(t *testing.T) {
	...
	if got, want := fmt.Sprint(keys), fmt.Sprint([]string{"a", "b", "c"}); got != want {
		t.Errorf("Expected %s after round trip, got %s", want, got)
	}
	if _, found := again.Search(TestRbTreeNode{S: "b"}); !found {
...
// TestJSONStructField marshals and unmarshals an RbTree nested in a
// struct through the encoding/json package.  The tree must be created
// with NewRbTree/NewRbTreeFunc before unmarshaling: for a nil *RbTree
