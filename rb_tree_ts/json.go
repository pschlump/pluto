...
// note that json.Marshal on a nil *RbTree never reaches this method —
...
// unmarshaling.  Elements decoded as duplicates of one another collapse,
// exactly as repeated Insert calls would.
//
...
func (tt *RbTree[T]) UnmarshalJSON(data []byte) error {
	...
	if len(items) > 0 {
		if tt == nil {
			panic("rb_tree_ts: UnmarshalJSON called on a nil tree")
		}
		if tt.cmp == nil {
			panic("rb_tree_ts: UnmarshalJSON called on a tree with no comparison function (create the tree with NewRbTree or NewRbTreeFunc)")
		}
	}
	if tt == nil {
