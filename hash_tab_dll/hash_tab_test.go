...
// Fixed-size hash table with doubly linked list buckets.
...
func TestInsertManyThenDeleteAll(t *testing.T) {
	...
	if ht.Len() != N {
		t.Fatalf("Expected length of %d, got %d", ht.Len(), N)
	}
	...
	if ht.Len() != N {
		t.Fatalf("Expected length of %d after duplicate insert, got %d", ht.Len(), N)
	}
	for i := range N {
	...
	if ht.Len() != N/2 {
		t.Fatalf("Expected length of %d, got %d", ht.Len(), N/2)
	}
