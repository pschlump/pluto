...
func TestEmptyTableOps(t *testing.T) {
	...
	if ht.ItemExists(&TestData{S: "nope"}) {
...
// TestDeleteFound verifies Search followed by DeleteFound removes the element.
func TestDeleteFound(t *testing.T) {
	...
	if ht.Len() != 19 {
		t.Errorf("Expected length 19, got %d", ht.Len())
	}
	if ht.ItemExists(&TestData{S: "   7"}) {
		t.Errorf("Expected element to be gone after DeleteFound")
	}
}
...
// TestWalk verifies the Walk callback visits every element and that the
// walk stops when the callback returns true.
func TestWalk(t *testing.T) {
	...
	if count != n {
		t.Errorf("Expected Walk to visit %d elements, got %d", n, count)
	}
	if el != nil || pos != -1 {
		t.Errorf("Expected Walk with no match to return nil, -1; got %v, %d", el, pos)
	}

	// Early stop when callback returns true.
	count = 0
	el, pos = ht.Walk(func(p int, data TestData, userData interface{}) bool {
		count++
		return data.S == "   9"
	...
	if el == nil || pos < 0 {
		t.Errorf("Expected Walk to stop at match, got %v, %d", el, pos)
	}
	if count > n {
		t.Errorf("Expected Walk to stop early, visited %d", count)
	}
}
...
func BenchmarkDeleteFound(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		// Re-insert after delete to keep the table population steady.
		if el := ht.Search(&items[i%1024]); el != nil {
			ht.DeleteFound(el)
		}
		ht.Insert(&items[i%1024])
	}
