...
// reflected, and a DFSPaths is safe for concurrent reads.
...
// NewDFSPaths snapshots the graph's adjacency lists under the read lock,
// then runs depth-first search from s lock-free on the snapshot, recording
// a path from s to every reachable vertex.  It panics on a nil or empty
// graph or on an out-of-range source — a programmer error with no sane
// answer.
// Complexity is O(V+E) time, O(V) space.
func NewDFSPaths(g *Graph, s int) *DFSPaths {
	if g == nil {
		panic("graph_ts: NewDFSPaths called on a nil graph")
	}
	g.lock.RLock()
	if g.n == 0 {
		g.lock.RUnlock()
		panic("graph_ts: NewDFSPaths called on an empty graph")
	}
	if s < 0 || s >= g.n {
		g.lock.RUnlock()
		panic(fmt.Sprintf("graph_ts: NewDFSPaths called with out-of-range source %d (graph has %d vertices)", s, g.n))
	}
	adj := g.snapshotAdj()
	g.lock.RUnlock()
	p := &DFSPaths{marked: make([]bool, len(adj)), edgeTo: make([]int, len(adj)), s: s}
	p.dfs(adj, s)
	return p
}
