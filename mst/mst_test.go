...
func parseEdgeWeightedGraph(t *testing.T, data string) *dijkstra.EdgeWeightedGraph {
	t.Helper()
	...
	for i := 0; i < m; i++ {
		...
		if !g.AddEdge(dijkstra.Edge{V: v, W: w, Weight: wt}) {
...
func edgeSet(edges []dijkstra.Edge) map[edgePair]bool {
...
func checkForest(t *testing.T, name string, n int, g *dijkstra.EdgeWeightedGraph, edges []dijkstra.Edge) {
	t.Helper()
...
func checkTinyEWG(t *testing.T, name string, g *dijkstra.EdgeWeightedGraph, edges []dijkstra.Edge, weight float64, length int) {
	t.Helper()
	if length != 7 {
...
func TestTinyEWGAgreement(t *testing.T) {
	...
	if len(edgeSet(lp.Edges())) != 7 || len(edgeSet(pr.Edges())) != 7 || len(edgeSet(kr.Edges())) != 7 {
		t.Errorf("Expected 7 MST edges from each algorithm.")
	}
...
func TestDisconnectedForest(t *testing.T) {
	...
	g := dijkstra.NewEdgeWeightedGraph(6)
	g.AddEdge(dijkstra.Edge{V: 0, W: 1, Weight: 1})
	g.AddEdge(dijkstra.Edge{V: 1, W: 2, Weight: 2})
	g.AddEdge(dijkstra.Edge{V: 0, W: 2, Weight: 9}) // cycle edge, must be skipped
	g.AddEdge(dijkstra.Edge{V: 3, W: 4, Weight: 3})
...
func TestSelfLoopAndParallelEdges(t *testing.T) {
	g := dijkstra.NewEdgeWeightedGraph(3)
	g.AddEdge(dijkstra.Edge{V: 0, W: 0, Weight: -5}) // even a cheap self-loop is never a tree edge
	g.AddEdge(dijkstra.Edge{V: 0, W: 1, Weight: 9})
	g.AddEdge(dijkstra.Edge{V: 0, W: 1, Weight: 4}) // parallel, cheaper
	g.AddEdge(dijkstra.Edge{V: 1, W: 2, Weight: 3})

	lp, pr, kr := NewLazyPrimMST(g), NewPrimMST(g), NewKruskalMST(g)
	for name, edges := range map[string][]dijkstra.Edge{
...
func TestNegativeWeights(t *testing.T) {
	...
	g.AddEdge(dijkstra.Edge{V: 0, W: 3, Weight: 4})
...
func TestSingleVertex(t *testing.T) {
	g := dijkstra.NewEdgeWeightedGraph(1)
	lp, pr, kr := NewLazyPrimMST(g), NewPrimMST(g), NewKruskalMST(g)
...
func TestImmutableResult(t *testing.T) {
	g := dijkstra.NewEdgeWeightedGraph(3)
	g.AddEdge(dijkstra.Edge{V: 0, W: 1, Weight: 10})
	g.AddEdge(dijkstra.Edge{V: 1, W: 2, Weight: 10})
	kr := NewKruskalMST(g)
	g.AddEdge(dijkstra.Edge{V: 0, W: 2, Weight: 1}) // a much cheaper route
...
func TestMSTPanics(t *testing.T) {
	var nilGraph *dijkstra.EdgeWeightedGraph
	zeroGraph := &dijkstra.EdgeWeightedGraph{}

	expectPanic(t, "NewLazyPrimMST on a nil graph", "NewLazyPrimMST", func() { NewLazyPrimMST(nilGraph) })
	...
	expectPanic(t, "NewPrimMST on a nil graph", "NewPrimMST", func() { NewPrimMST(nilGraph) })
	expectPanic(t, "NewPrimMST on a zero-value graph", "nil or empty graph", func() { NewPrimMST(zeroGraph) })
	expectPanic(t, "NewKruskalMST on a nil graph", "NewKruskalMST", func() { NewKruskalMST(nilGraph) })
...
func BenchmarkKruskalMST(b *testing.B) {
	g := benchmarkGraph(4096, 16384, 42)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		NewKruskalMST(g)
	}
...
func BenchmarkLazyPrimMST(b *testing.B) {
	g := benchmarkGraph(4096, 16384, 42)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		NewLazyPrimMST(g)
