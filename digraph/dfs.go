...
// NewDirectedDFS runs depth-first search from every source and records
// the reachable vertices.  It panics on a nil or empty digraph, with no
// sources, or on an out-of-range source — a programmer error with no
// sane answer.
// Complexity is O(V+E) time, O(V) space.
func NewDirectedDFS(g *Digraph, sources ...int) *DirectedDFS {
	if g == nil || g.n == 0 {
		panic("digraph: NewDirectedDFS called on a nil or empty digraph")
	}
	if len(sources) == 0 {
		panic("digraph: NewDirectedDFS called with no sources, need at least one")
	}
	for _, s := range sources {
		if s < 0 || s >= g.n {
			panic(fmt.Sprintf("digraph: NewDirectedDFS called with out-of-range source %d (digraph has %d vertices)", s, g.n))
		}
	}
	d := &DirectedDFS{marked: make([]bool, g.n)}
	for _, s := range sources {
		if !d.marked[s] {
			d.dfs(g.adj, s)
		}
...
// NewDFSDirectedPaths runs depth-first search from s and records a
// (directed) path from s to every reachable vertex.  It panics on a nil
// or empty digraph or on an out-of-range source — a programmer error
// with no sane answer.
// Complexity is O(V+E) time, O(V) space.
func NewDFSDirectedPaths(g *Digraph, s int) *DFSDirectedPaths {
	if g == nil || g.n == 0 {
		panic("digraph: NewDFSDirectedPaths called on a nil or empty digraph")
	}
	if s < 0 || s >= g.n {
		panic(fmt.Sprintf("digraph: NewDFSDirectedPaths called with out-of-range source %d (digraph has %d vertices)", s, g.n))
	}
	p := &DFSDirectedPaths{marked: make([]bool, g.n), edgeTo: make([]int, g.n), s: s}
	p.dfs(g.adj, s)
	return p
}
