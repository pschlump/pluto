	...
	for _, data := range [][]int{nil, {}} {
		func() {
			defer func() {
				...
				if msg, ok := r.(string); !ok || !strings.Contains(msg, "segment_tree_ts: NewSegmentTree") {
		...
		func() {
			defer func() {
				...
				if msg, ok := r.(string); !ok || !strings.Contains(msg, "segment_tree_ts: NewSegmentTreeFunc") {
...
func TestNilUpdatePanics(t *testing.T) {
	...
	defer func() {
		...
		if msg, ok := r.(string); !ok || !strings.Contains(msg, "segment_tree_ts: Update") {
...
func TestLockNlCompound(t *testing.T) {
	...
	st.Lock()
	if st.NlLen() != 5 {
		t.Errorf("Expected NlLen()=5, got %d", st.NlLen())
	}
	v, ok := st.NlValue(2)
	if !ok || v != 3 {
	...
	if !st.NlUpdate(2, 2*v) {
		t.Errorf("Expected NlUpdate(2, 6) to return true.")
	}
	if s, ok := st.NlQuery(0, 2); !ok || s != 9 {
		t.Errorf("Expected NlQuery(0,2)=(9, true) after doubling slot 2, got (%d, %v)", s, ok)
	}
	s, ok := st.NlQuery(0, st.NlLen()-1)
	st.Unlock()

	if !ok || s != 18 {
		t.Errorf("Expected NlQuery(0,4)=(18, true), got (%d, %v)", s, ok)
	}
	if got, _ := st.Query(0, 4); got != 18 {
		t.Errorf("Expected Query(0,4)=18 after the compound update, got %d", got)
	}

	// Out-of-range Nl operations report, they do not panic.
	st.Lock()
	if st.NlUpdate(5, 1) {
		t.Errorf("Expected out-of-range NlUpdate to return false.")
	}
	if _, ok := st.NlValue(5); ok {
		t.Errorf("Expected NlValue(5) to return ok=false.")
	}
	if _, ok := st.NlQuery(3, 2); ok {
		t.Errorf("Expected NlQuery(3,2) to return ok=false.")
	}
	st.Unlock()
}
