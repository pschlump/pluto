...
// -------------------------------------------------------------------------------------------------------
// All — ascending iteration over a snapshot
// -------------------------------------------------------------------------------------------------------
...
// TestNilLockUnlock verifies that Lock/Unlock on a nil trie are no-ops.
...
// nil *Tst.  The panic fires before any lock acquisition — a nil
// receiver has no lock to take.
func TestNilInsertPanics(t *testing.T) {
	var nilTst *Tst[int]
	expectPanic(t, "Insert on a nil *Tst", "tst_ts: Insert called on a nil Tst", func() {
		nilTst.Insert("a", 1)
	})
}
