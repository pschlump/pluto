...
func TestWalkFuncAndAll(t *testing.T) {
	...
	// All takes a snapshot, so it is safe to mutate the table while
	// iterating (it would deadlock if it held the read lock).
	for item := range ht.All() {
		ht.Insert(item)
	}
	if ht.Len() != total {
		t.Errorf("Expected length of %d after insert-during-All, got %d", total, ht.Len())
	}
}
...
func TestNlAPI(t *testing.T) {
	...
	// The Nl-prefixed methods run without taking the lock; the caller
	// holds it across a batch of operations.
	ht.ReadLock()
...
func TestConcurrentAccess(t *testing.T) {
	...
	for w := 0; w < workers; w++ {
		...
		go func(w int) {
			...
			for i := 0; i < perWorker; i++ {
				item := TestData{S: fmt.Sprintf("w-%d-i-%d", w, i)}
				ht.Insert(&item)
				if !ht.ItemExists(&item) {
					t.Errorf("Expected to find just-inserted item %q", item.S)
				}
				if got := ht.Search(&item); got == nil {
					t.Errorf("Expected Search to find %q", item.S)
				}
				if i%2 == 0 {
					if !ht.Delete(&item) {
						t.Errorf("Expected Delete to remove %q", item.S)
					}
				}
			}
		}(w)
	...
	// Half of the items (the odd-numbered ones) remain.
	want := workers * perWorker / 2
	if ht.Len() != want {
		t.Errorf("Expected length of %d after concurrent run, got %d", want, ht.Len())
	}

	// Iterate the result concurrently with reads.
	n := 0
	for range ht.All() {
		n++
	}
	if n != want {
		t.Errorf("Expected All to yield %d elements, got %d", want, n)
	}
}
