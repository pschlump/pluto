...
// Thread-safe fixed-size hash table with a binary search tree per bucket.
...
// TestData is the standard test element: S is the key that the comparison
// and hash functions read; N is satellite data both ignore, used to verify
// that duplicate inserts replace the stored value.
type TestData struct {
	S string
	N int
}

// newTestTab builds a table with deterministic FNV-1a-64 hashing over S and
// string ordering over S, so tests can reason about bucket placement and
// the in-order (ascending) visit inside a bucket.  The default NewHashTab
// hashes with a per-process random maphash seed, which is fine for
// membership assertions but not for placement.
func newTestTab(n int) *HashTab[TestData] {
	return NewHashTabFunc(
		func(a, b TestData) int { return strings.Compare(a.S, b.S) },
...
// TestDefaultConstructor exercises NewHashTab with the builtin < / >
// ordering and the randomized maphash hashing.  Bucket positions vary per
// process, so only membership is asserted.
func TestDefaultConstructor(t *testing.T) {
	...
	// The ordered constructor also takes floats and integers.
	nums := NewHashTab[float64](9)
	nums.Insert(2.5)
	nums.Insert(-1.5)
	nums.Insert(2.5) // duplicate
	if nums.Len() != 2 {
		t.Errorf("Expected 2 floats, got %d", nums.Len())
	}
	if v, found := nums.Search(-1.5); !found || v != -1.5 {
		t.Errorf("Expected to find -1.5, got %v found=%v", v, found)
	}
	if !nums.Delete(2.5) || nums.Len() != 1 {
		t.Errorf("Expected delete of 2.5 to leave 1 element")
	}
...
// TestDumpOutput verifies Dump writes the header, one line per empty bucket
// and one line per stored element (a bucket tree prints one line per
...
func TestDumpOutput(t *testing.T) {
	...
	// Element lines name the bucket, empty buckets are marked empty.
	occupied, empty := 0, 0
...
// and that Truncate empties every bucket tree (no stale elements).
func TestTruncateReuse(t *testing.T) {
	...
	for i, tree := range ht.buckets {
		if !tree.IsEmpty() || tree.Len() != 0 {
			t.Fatalf("Expected empty bucket tree after Truncate, bucket %d is not", i)
		}
	}
...
// the tree insert/replace/delete paths under heavy collision (size 7, keys
// 0..399).
...
// bucket trees) and then searches for and deletes every one of them,
// verifying each step.
