...
// TestLongestPrefixOf covers the prefix-of-query query: the longest
// stored key that is a prefix of the argument.
func TestLongestPrefixOf(t *testing.T) {
	var pt PatriciaTrie[int]
	for i, k := range []string{"she", "shells", "sea", "shore", "the"} {
		pt.Insert(k, i)
	}
	...
	cases := []struct {
		...
		{"shells", "shells", true},     // the query itself, stored
		{"shellshock", "shells", true}, // a stored key extended
		{"shell", "she", true},         // a shorter stored key is the prefix
		{"sea lion", "sea", true},
		{"theoretical", "the", true},
		{"the", "the", true},
		{"sh", "", false}, // prefixes of stored keys cut the other way
		{"s", "", false},
		{"xyz", "", false},
		{"", "", false}, // no empty key stored
	}
	for _, c := range cases {
		got, v, ok := pt.LongestPrefixOf(c.query)
		if got != c.want || ok != c.ok {
			t.Errorf("LongestPrefixOf(%q)=(%q, %v), want (%q, %v)", c.query, got, ok, c.want, c.ok)
			continue
		}
		if ok {
			wantV, _ := pt.Search(c.want)
			if v != wantV {
				t.Errorf("LongestPrefixOf(%q) value=%d, want %d", c.query, v, wantV)
			}
		}
	}

	// A stored chain of prefixes: every extension resolves to the
	// longest member present.
	pt2 := PatriciaTrie[int]{}
	pt2.Insert("a", 1)
	pt2.Insert("ab", 2)
	pt2.Insert("abc", 3)
	for q, want := range map[string]string{"abcd": "abc", "abc": "abc", "abx": "ab", "ab": "ab", "ax": "a", "a": "a", "b": ""} {
		got, _, ok := pt2.LongestPrefixOf(q)
		if want == "" {
			if ok {
				t.Errorf("LongestPrefixOf(%q)=(%q, true), want not-found", q, got)
			}
			continue
		}
		if got != want || !ok {
			t.Errorf("LongestPrefixOf(%q)=(%q, %v), want (%q, true)", q, got, ok, want)
		}
	}

	// Deleting a member shortens the answer.
	pt2.Delete("abc")
	if got, _, ok := pt2.LongestPrefixOf("abcd"); !ok || got != "ab" {
		t.Errorf("After deleting \"abc\", LongestPrefixOf(\"abcd\")=(%q, %v), want (\"ab\", true)", got, ok)
	}
}

// TestLongestPrefixOfEmptyAndNulKeys: the empty key is a prefix of
// everything, and NUL-padded key chains resolve through the prefix-free
// encoding.
func TestLongestPrefixOfEmptyAndNulKeys(t *testing.T) {
	var pt PatriciaTrie[int]
	pt.Insert("", 0)
	pt.Insert("a", 1)
	pt.Insert("a\x00", 2)

	if got, _, ok := pt.LongestPrefixOf("anything"); !ok || got != "a" {
		t.Errorf(`Expected "a" (longer than the empty key): got (%q, %v)`, got, ok)
	}
	if got, _, ok := pt.LongestPrefixOf("a\x00\x00"); !ok || got != "a\x00" {
		t.Errorf("Expected the NUL-padded key \"a\\x00\", got (%q, %v)", got, ok)
	}
	if got, _, ok := pt.LongestPrefixOf("aX"); !ok || got != "a" {
		t.Errorf("Expected \"a\" for query \"aX\", got (%q, %v)", got, ok)
	}
	if got, _, ok := pt.LongestPrefixOf(""); !ok || got != "" {
		t.Errorf("Expected the stored empty key for query \"\", got (%q, %v)", got, ok)
	}
}

// TestLongestPrefixOfNilAndZero: nil and zero-value tries report
// not-found; a populated trie reports not-found for queries nothing
// prefixes.
func TestLongestPrefixOfNilAndZero(t *testing.T) {
	var nilTrie *PatriciaTrie[int]
	if got, _, ok := nilTrie.LongestPrefixOf("a"); ok {
		t.Errorf("Expected not-found on a nil trie, got (%q, true)", got)
	}
	var pt PatriciaTrie[int]
	if got, _, ok := pt.LongestPrefixOf("a"); ok {
		t.Errorf("Expected not-found on a zero-value trie, got (%q, true)", got)
	}
	pt.Insert("xyz", 1)
	if got, _, ok := pt.LongestPrefixOf("a"); ok {
		t.Errorf("Expected not-found when no stored key prefixes the query, got (%q, true)", got)
	}
	if got, _, ok := pt.LongestPrefixOf("xyzzy"); !ok || got != "xyz" {
...
// TestKeysThatMatch covers the glob query over a small key set, the
// early break, and the ordering.
func TestKeysThatMatch(t *testing.T) {
	...
	collect := func(pattern string) []string {
		var got []string
		for k, v := range pt.KeysThatMatch(pattern) {
			got = append(got, k)
			if want, _ := pt.Search(k); v != want {
				t.Errorf("KeysThatMatch yielded %q with value %d, want %d", k, v, want)
			}
		}
		return got
	}
	...
	if got := join(collect("s*")); got != "sea,sells,she,shells,shore" {
		t.Errorf("KeysThatMatch(\"s*\")=%q", got)
	}
	if got := join(collect("s????")); got != "sells,shore" {
		t.Errorf("KeysThatMatch(\"s????\")=%q", got)
	}
	if got := join(collect("s??")); got != "sea,she" {
		t.Errorf("KeysThatMatch(\"s??\")=%q", got)
	}
	if got := join(collect("s???")); got != "" {
		t.Errorf("KeysThatMatch(\"s???\")=%q, want no 4-byte keys", got)
	}
	if got := join(collect("s[he]*")); got != "sea,sells,she,shells,shore" {
		t.Errorf("KeysThatMatch(\"s[he]*\")=%q", got)
	}
	if got := join(collect("[^s]*")); got != "the" {
		t.Errorf("KeysThatMatch(\"[^s]*\")=%q", got)
	}
	if got := join(collect("*e*")); got != "sea,sells,she,shells,shore,the" {
		t.Errorf("KeysThatMatch(\"*e*\")=%q", got)
	}
	if got := join(collect("sh?lls")); got != "shells" {
		t.Errorf("KeysThatMatch(\"sh?lls\")=%q", got)
	}
	// A literal '*' is escaped; the empty pattern matches only the empty
	// key; nothing matches a disjoint literal.
	if got := join(collect("s\\*")); got != "" {
		t.Errorf("KeysThatMatch(\"s\\\\*\")=%q, want no matches", got)
	}
	if got := join(collect("zzz")); got != "" {
		t.Errorf("KeysThatMatch(\"zzz\")=%q, want no matches", got)
	}
	if got := join(collect("")); got != "" {
		t.Errorf("KeysThatMatch(\"\")=%q, want no matches (no empty key stored)", got)
	}

	// The empty key matches only patterns that match "".
	pt.Insert("", 99)
	if got := join(collect("")); got != "" {
		t.Errorf("KeysThatMatch(\"\")=%q, want the empty key", got)
	}
	var n int
	for k := range pt.KeysThatMatch("") {
		n++
		_ = k
	}
	if n != 1 {
		t.Errorf("Expected exactly the empty key to match \"\", got %d", n)
	}

	// Breaking out of the loop stops the walk early.
	count := 0
	for range pt.KeysThatMatch("*") {
		count++
		if count == 2 {
			break
		}
	...
	var first string
	for k := range pt.KeysThatMatch("s??") {
		first = k
		break
	}
	if first != "sea" {
		t.Errorf("Expected a single-variable range to yield \"sea\", got %q", first)
	}
}

// TestKeysThatMatchNilAndZero: nil and zero-value tries iterate as
// empty ones.
func TestKeysThatMatchNilAndZero(t *testing.T) {
	var nilTrie *PatriciaTrie[int]
	for range nilTrie.KeysThatMatch("*") {
		t.Errorf("Expected KeysThatMatch on a nil trie to visit nothing.")
	}
	var pt PatriciaTrie[int]
	for range pt.KeysThatMatch("*") {
		t.Errorf("Expected KeysThatMatch on a zero-value trie to visit nothing.")
	}
}

// TestKeysThatMatchBinaryKeys: glob matching is on raw bytes — NUL and
// high bytes are ordinary key bytes.
func TestKeysThatMatchBinaryKeys(t *testing.T) {
	var pt PatriciaTrie[int]
	keys := []string{"\x00", "\x00\x01", "\xff\xfe", "a\x00b"}
	for i, k := range keys {
		pt.Insert(k, i)
	}
	var got []string
	for k := range pt.KeysThatMatch("\x00*") {
		got = append(got, k)
	}
	if strings.Join(got, ",") != "\x00,\x00\x01" {
		t.Errorf("Expected the two NUL keys, got %q", got)
	}
	got = nil
	for k := range pt.KeysThatMatch("[\x80-\xff]?") {
		got = append(got, k)
	}
	if strings.Join(got, ",") != "\xff\xfe" {
		t.Errorf("Expected the high-byte key, got %q", got)
	}
}
