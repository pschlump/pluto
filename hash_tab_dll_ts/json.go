...
func (tt *HashTab[T]) UnmarshalJSON(data []byte) error {
	...
	if len(items) > 0 {
		if tt == nil {
			panic("hash_tab_dll_ts: UnmarshalJSON called on a nil table")
		}
		if tt.eq == nil || tt.hash == nil {
			panic("hash_tab_dll_ts: UnmarshalJSON called on a table with no equality/hash functions (create the table with NewHashTab or NewHashTabFunc)")
		}
	}
	if tt == nil {
	...
	for _, b := range tt.buckets {
		b.Truncate() // empty every chain, releasing the elements for GC
	}
	tt.length = 0
