...
// MarshalJSON implements json.Marshaler so a Dll can be used directly
...
// UnmarshalJSON implements json.Unmarshaler so a Dll can be used
...
func (ns *Dll[T]) UnmarshalJSON(data []byte) error {
	var items []T
	...
	if len(items) > 0 {
		if ns == nil {
			panic("dll_ts: UnmarshalJSON called on a nil list")
		}
		if ns.eq == nil {
			panic("dll_ts: UnmarshalJSON called on a list with no equality function (create the list with NewDll or NewDllFunc)")
		}
	}
	if ns == nil {
