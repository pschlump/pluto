...
a doubly-linked link:https://en.wikipedia.org/wiki/Skip_list[skip list].  It
is a variant of `skip_list` in which every node also carries a back pointer
on level 0, so the bottom level is a doubly-linked list.  Items must
implement `github.com/pschlump/pluto/comparable.Comparable` (a single
`Compare` method).
