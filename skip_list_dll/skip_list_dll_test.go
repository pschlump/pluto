...
// checks the list against a sorted reference slice in both directions, then
// deletes them all in random order, verifying the level-0 back chain after
// every delete.
func TestListRandomized(t *testing.T) {
	...
	checkBothWays := func(tag string) {
		fwd := make([]string, 0, List1.Length())
		for v := range List1.All() {
			fwd = append(fwd, v.S)
		}
		if !sort.StringsAreSorted(fwd) {
			t.Fatalf("%s: All: items are not in ascending order", tag)
		}
		bwd := make([]string, 0, List1.Length())
		for v := range List1.Backward() {
			bwd = append(bwd, v.S)
		}
		if len(bwd) != len(fwd) {
			t.Fatalf("%s: Backward yielded %d items, All yielded %d", tag, len(bwd), len(fwd))
		}
		for i := range fwd {
			if bwd[len(bwd)-1-i] != fwd[i] {
				t.Fatalf("%s: Backward does not mirror All at position %d", tag, i)
			}
		}
	}
	checkBothWays("after insert")
	...
	for i, n := range del {
		...
		if i%1000 == 0 {
			checkBothWays(fmt.Sprintf("after %d deletes", i+1))
		}
	}
	if !List1.IsEmpty() {
